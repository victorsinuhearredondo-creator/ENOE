...
# -*- coding: utf-8 -*-
"""
04N_validacion_v3.py
====================
Valida parquets ENOE Nueva (2023T1-2025T4) contra tabulados oficiales INEGI.

Fix vs v2: el parser de tabla-ancha (parse_wide_table) era incorrecto.
Los XLS de la Nueva ENOE tienen la MISMA estructura vertical que la tradicional:
  - Col 0 = nombre del indicador
  - Col 1 = valor Total nacional
  - El label puede no estar en col 0 exacta (celdas combinadas)

Solución: usa el mismo parser de 04T_validacion_v6 (find_label_get_total).
Diferencias vs 04T:
  - Parquets en derivados_nueva/  con nombre enoe_nueva_YYYY_TQ.parquet
  - Ponderador: FAC_TRI (nueva) en vez de FAC (tradicional)
  - Engine: xlrd para .xls (igual que tradicional)
"""
...
# Labels — mismos que 04T_validacion_v6
...
def find_label_get_total(df: pd.DataFrame, labels: list):
    """
    Busca cualquiera de los labels en las primeras 8 cols de cada fila.
    Cuando lo encuentra en columna j, devuelve el primer número a la derecha de j.
    """
...
# ============================================================
# EXCEL — fuerza xlrd para .xls (igual que v6)
# ============================================================
def require_xlrd():
    try:
        import xlrd  # noqa
    except ImportError:
        print("\n  [ERROR] xlrd no instalado.")
...
def read_sheet(path: Path, sheet, nrows=None) -> pd.DataFrame:
    return pd.read_excel(path, sheet_name=sheet,
                         header=None, nrows=nrows, engine=get_engine(path))

def get_sheet_names(path: Path) -> list:
    try:
        return pd.ExcelFile(path, engine=get_engine(path)).sheet_names
...
def compute_micro_nueva(path: Path) -> dict:
    df = pd.read_parquet(path)
    # Nueva ENOE usa FAC_TRI; fallback a FAC si no existe
    fac    = pick_col(df, ["FAC_TRI","FAC_MEN","FAC","FAC_P","FACTOR"])
...
def main():
    ...
    parquet_d   = base_dir / "derivados_nueva"
    valid_d     = base_dir / "validadores_2" / "extract" / "nueva"
    out_dir     = base_dir / "diagnosticos"
    out_dir.mkdir(parents=True, exist_ok=True)

    print("=" * 62)
    print("  04N_validacion_v3.py — Micro vs INEGI (Nueva ENOE)")
    print("=" * 62)
    print(f"  BASE_DIR    : {base_dir}")
    print(f"  PARQUET_DIR : {parquet_d}")
    print(f"  VALID_DIR   : {valid_d}")
    print(f"  UMBRAL_PP   : {UMBRAL_PP} pp")

    require_xlrd()
    print("  xlrd: OK")

    # ── Micro ─────────────────────────────────────────────────
    parquets = sorted(parquet_d.glob("enoe_nueva_*_T*.parquet"))
    if not parquets: parquets = sorted(parquet_d.glob("enoe_nueva_*_T*.csv.gz"))
    if not parquets:
    ...
    micro_rows = []
    for f in parquets:
        m = re.search(r"enoe_nueva_(\d{4})_T([1-4])", f.name, re.IGNORECASE)
        if not m: continue
        periodo = f"{m.group(1)}T{m.group(2)}"
        print(f"  micro {periodo}...", end="  ")
        stats = compute_micro_nueva(f)
        stats["periodo"] = periodo
        micro_rows.append(stats)
        if "error" in stats:
            print(f"[WARN] {stats['error']}")
        else:
            print(f"TPART={stats['TPART_micro']:.4f}  TDESOC={stats['TDESOC_micro']:.4f}"
                  f"  (fac={stats['fac_col']})")
    ...
    if not valid_d.exists():
        ...
        micro_df.to_csv(out_dir/"04N_micro_sin_oficial.csv",
    ...
    print(f"\n  Periodos micro    : {sorted(micro_df['periodo'].dropna().tolist())}")
    print(f"  Periodos oficiales: {sorted(oficial_df['periodo'].tolist())}")
    ...
    out_csv = out_dir / "04N_validacion_nueva.csv"
    ...
    lines = [
        "=" * 62, "  04N VALIDACION NUEVA — Resumen", "=" * 62,
    ...
    txt = "\n".join(lines)
    print("\n" + txt)
    (out_dir / "04N_resumen_nueva.txt").write_text(txt, encoding="utf-8")
    print(f"\n  OK -> {out_csv}")
    print(f"  OK -> {out_dir}/04N_resumen_nueva.txt")
